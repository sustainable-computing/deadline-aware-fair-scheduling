...
for t in range(0, len(P[0,:])):
    ...
    for s in range(maxIteration):
        direction = np.dot(evMatrix.T,evPower)
        lamda = np.maximum(0.0,lamda - gamma1*(A-direction))
        mu = np.maximum(0.0, mu - gamma2*(theta*A-direction))
        
        direction = np.dot(evMatrix,mu)
        aux.w_update(Laxity, urgent)
        y = np.minimum(aux.w*np.reciprocal(direction), UB)
        
        direction = np.dot(evMatrix,lamda)
        evPower = np.minimum(np.maximum(y, np.reciprocal(direction)), UB)
        if np.linalg.norm(prev-evPower) <= tol:
            print('yes')
            break
        prev = evPower
