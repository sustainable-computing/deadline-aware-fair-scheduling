...
for h in tqdm(range(simu_params['start_hr'], simu_params['end_hr'])):
    ...
    for t in range(0, n_slots):
        for key in algo_list:
            ...
            if index == simu_params['begin_index']:
                algo_list['decentral'].current_slot = algo_list['central'].current_slot
                algo_list['decentral'].remaining_demand = algo_list['central'].remaining_demand
                
                result['decentral']= algo_list['decentral'].convergence(P,Q, result['central'][index])
