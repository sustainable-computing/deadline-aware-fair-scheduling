...
class algo:
    def __init__(self, DSSObj, env, P, Q, slot_len_in_min=10, start_hr=0, mode='change', params={}):
        ...
        self.max_rate = 1+60.0*np.array(env['demand']) # In kWsec

        self.arrival = np.array(self.env['evArrival'])/(60*slot_len_in_min)
        self.duration = np.array(self.env['evDuration'])/(60*slot_len_in_min)
        self.claimed_duration = np.array(self.env['evClaimedDuration'])/(60*slot_len_in_min)
